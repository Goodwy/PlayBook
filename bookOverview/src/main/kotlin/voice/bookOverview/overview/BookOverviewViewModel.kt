...
constructor(
  ...
  internal fun state(): BookOverviewViewState {
    ...
    val miniPlayerStyle = miniPlayerStylePref.flow.collectAsState(initial = 0).value
    ...
    return BookOverviewViewState.Content(
      ...
      cover = if (useChapterCover) currentBook?.currentChapter?.cover?.let(::ImmutableFile) else currentBook?.content?.cover?.let(::ImmutableFile),
